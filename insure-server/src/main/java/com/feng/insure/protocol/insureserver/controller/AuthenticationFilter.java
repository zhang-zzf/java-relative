...
public class AuthenticationFilter implements Filter {
    ...
    private boolean verifyRequest(HttpServletRequest httpServletRequest) {
        ...
        signBody.append(X_REQUEST_TIMESTAMP).append(": ")
            .append(httpServletRequest.getHeader(X_REQUEST_TIMESTAMP)).append("\r\n");
        ...
        if (HttpMethod.GET.name().equals(method)) {
            // get
            final Optional<CharSequence> sortedQueryParam = httpServletRequest.getParameterMap()
                .entrySet().stream()
                .sorted(Comparator.comparing(Map.Entry::getKey))
                .map(e -> CollectionFormat.CSV.join(e.getKey(), Arrays.asList(e.getValue()),
        ...
        log.info("request to verify: publicKey={}, signature={}, req={}", publicKey, signature,
            requestBody);
        final boolean verify = cipherUtil.md5WithRSAVerify(
            ...
            publicKey);
        if (!verify) {
            log.error("request verify failed: publicKey={}, signature={}, req={}", publicKey, signature,
                requestBody);
        }
        return verify;
    }
    ...
    public enum CollectionFormat {
         ...
         *
         * <p>
         * Null values are treated somewhat specially. With EXPLODED, the field is repeated without any "=" for
         * backwards compatibility. With all other formats, null values are not included in the joined value list.
         * </p>
         *
         ...
         * @return The formatted char sequence of the field and joined values. If the value collection is empty, an
         * empty char sequence will be returned.
         */
