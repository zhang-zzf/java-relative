...
public class RedisDistributedLock {
    ...
    public void unlock(String lockKey, String myRandomValue) {
        RedisConnection conn = RedisConnectionUtils.getConnection(redisConnectionFactory);
        try {
            Long keyDeletedNum = conn.eval(UNLOCK_LUA, ReturnType.INTEGER, 1, keySerialized.serialize(lockKey),
                valueSerialized.serialize(myRandomValue));
            if (keyDeletedNum == 0) {
                throw new IllegalStateException("the lock is not exists or has been expired, so your protected " +
    ...
    public void lockInterruptibly(String lockKey, String myRandomValue, long expirationTimeInSeconds) throws InterruptedException {
        for (boolean locked = false; !locked; ) {
            // 响应线程中断
