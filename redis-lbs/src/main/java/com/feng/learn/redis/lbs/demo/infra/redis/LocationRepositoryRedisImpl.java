...
public class LocationRepositoryRedisImpl implements LocationRepository {
    ...
    public void updateLocation(List<Location> locations) {
        ...
        List<Location> curLocations = batchGetById(locations.stream().map(Location::getId).collect(toList()));
        // 更新 Geo Index
    ...
    private void updateGeoIndexUsePipeline(List<Location> locations) {
        Map<String, List<Location>> keys2Data = locations.stream().collect(groupingBy(this::locationToQueue, toList()));
    ...
    private void removeFromGeoForQueueChangedLocation(List<Location> locations, List<Location> curData) {
        Map<Long, Location> id2Location = locations.stream().collect(toMap(Location::getId, Function.identity()));
    ...
    public List<Long> searchGeo(double lat, double lon, long radius) {
        ...
        List<GeoResults<GeoLocation<byte[]>>> geoResults = pipelineSearchGeo(queues, new Circle(lon, lat, radius));
    ...
    private List<GeoResults<GeoLocation<byte[]>>> pipelineSearchGeo(Set<String> queues, Circle within) {
        return redisTemplate.executePipelined((RedisCallback<GeoResults<GeoLocation<byte[]>>>) connection -> {
