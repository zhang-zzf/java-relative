...
public class AsyncRabbitConsumerConfiguration {
   ...
   * @param cf
   * @return
   * @Bean(RABBIT_TEMPLATE) public RabbitTemplate rabbitTemplate(@Qualifier(C_F) ConnectionFactory
   * cf) { RabbitTemplate template = new RabbitTemplate(cf); return template; }
   */
