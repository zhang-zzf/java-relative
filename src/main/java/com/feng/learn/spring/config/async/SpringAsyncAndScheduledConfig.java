...
@Configuration
@Import({
    // 导入其他spring异步线程池，集中管理
    AsyncThreadPoolForSomeTaskConfig.class,
...
public class SpringAsyncAndScheduledConfig {
    ...
    public Object addInfoToSpringAsyncTask(ProceedingJoinPoint pjp, Callable callable) throws Throwable {
        String key = "Hello, World";
        log.info("{} set: key => {}", Thread.currentThread(), key);
        Callable taskWithTrace = () -> {
            log.info("{} get: key => {}", Thread.currentThread(), key);
    ...
    public Object addInfoToSpringTaskExecutor(ProceedingJoinPoint pjp, Runnable runnable) throws Throwable {
        String key = "Hello, World";
        log.info("{} set: key => {}", Thread.currentThread(), key);
